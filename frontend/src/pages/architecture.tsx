...
export default function ArchitectureStudio() {
  ...
  const { data: docs, isLoading } = useListArchitectureDocs({
    query: {
      queryKey: getListArchitectureDocsQueryKey(),
  ...
  // Find the active architecture job from context
  const activeJob = useMemo(
    () =>
      jobs.find(
        (j) => j.type === "architecture" && (j.status === "queued" || j.status === "generating"),
      ),
    [jobs],
  );
  ...
  useEffect(() => {
    if (globalStream.result) {
      const response = globalStream.result as { document?: ArchitectureDoc };
  ...
  useEffect(() => {
    ...
    const completedJob = jobs.find(
      (j) => j.type === "architecture" && j.status === "completed" && j.result,
    );
    if (completedJob) {
      const result = completedJob.result as { document?: ArchitectureDoc };
      if (result?.document) {
  ...
  const handleGenerate = useCallback(() => {
    ...
    const job = addJob({
      type: "architecture",
      title: `${DOC_TYPES.find((t) => t.id === docType)?.label || "Architecture"} - ${projectName || "New Project"}`,
      prompt,
      status: "queued",
      streamUrl: "/api/architecture/generate/stream",
      streamBody: body,
    });

    // Start the global SSE stream (persists across navigations)
    startGlobalStream({
      url: "/api/architecture/generate/stream",
      body,
      jobId: job.id,
      onChunk: (_chunk: string) => {
        // Update progress based on accumulated content
        const w = globalThis as Record<string, unknown>;
        const states = w.__streamStates as Map<string, { streamedContent: string }> | undefined;
        const state = states?.get(job.id);
        if (state?.streamedContent) {
          const pct = Math.min(
            95,
            Math.round((state.streamedContent.length / 8000) * 100),
          );
    ...
    setProjectName("");
    setDocType("full");
  }, [prompt, docType, projectName, isGenerating, addJob, updateJob]);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Design system architecture and generate technical specifications.
          </p>
        </div>
        {isGenerating && (
        ...
        <Card className="col-span-1 flex flex-col bg-card/50 backdrop-blur">
          ...
          <CardContent className="space-y-4 flex-1 flex flex-col min-h-0 overflow-y-auto">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={docType}
                onValueChange={(v: ArchitectureGenerateInputDocType) => setDocType(v)}
                disabled={isGenerating}
              >
                <SelectTrigger>
            ...
            {isGenerating && !activeJob && (
              <p className="text-xs text-muted-foreground text-center animate-pulse">
                Generating architecture document... This may take a moment.
              </p>
            )}
            {globalStream.error && (
          ...
          <CardContent className="flex-1 flex gap-4 min-h-0 p-4 pt-0">
            ...
            <div className="w-1/3 border-r border-border/50 pr-4 flex flex-col">
              ...
              <div className="space-y-2 overflow-y-auto flex-1 pr-2">
                ...
                {isGenerating && activeJob && (
                  <div className="w-full text-left p-3 rounded-lg border border-dashed border-purple-500/50 bg-purple-500/5 animate-pulse">
                    <div className="flex justify-between items-center">
                      <span className="font-medium text-sm truncate text-purple-500">
                        {activeJob.title}
                      </span>
                ...
                {isLoading ? (
                  ...
                  <EmptyState
                    ...
                    description="Generate your first architecture document to see it here."
                  />
                ) : (
                  docs?.map((doc) => (
                    <button
                    ...
                    >
                      <div className="font-medium text-sm truncate flex items-center gap-2">
                        {createElement(
                          DOC_TYPES.find((t) => t.id === doc.docType)?.icon || Layers,
            ...
            <div className="w-2/3 pl-2 flex flex-col min-h-0">
                ...
                <div className="flex-1 flex flex-col min-h-0">
                  {streamedDoc ? (
                    <>
                      ...
                      <div className="flex-1 overflow-y-auto rounded-lg border border-purple-500/20 bg-background/50 p-4">
              ...
              {!isGenerating && !selectedDoc && (
                <div className="h-full flex flex-col items-center justify-center text-muted-foreground opacity-50">
                  ...
                  <p className="text-sm">Generate or select an architecture document</p>
                </div>
              )}
            </div>
          </CardContent>
